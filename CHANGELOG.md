...
<b-table :data="myData">
    ...
    <b-table-column field="age" label="Age">
        <template v-slot:default="props"> {{ props.row.age }} </template>
    </b-table-column>
</b-table>
```
...
```html
<b-table :data="tableData" :row-class="row => row.amount < 10 && 'is-warning'"
    >[...]</b-table
>
```
...
- **Field**, **Snackbar** and **Toast** `position` property now needs the `is-` prefix (eg. `is-top-left` or `is-centered`) — _Gotta fix these inconsistencies while it's still in beta_
...
```html
<b-table :data="data" :selected.sync="selectedRow"></b-table>
```
